...
SEARCH=${SEARCH// /%20}
TITLE=$(curl "https://www.bbfc.co.uk/search?q=$SEARCH&s=Relevance&t%5B%5D=Film" | grep -oP -m 1 '(?<=href="/release/).*?(?=")' | head -1)
LINK=$(curl "https://www.bbfc.co.uk/search?q=$SEARCH&s=Relevance&t%5B%5D=Film" | grep -oP -m 1 '(?<=href="/release/).*?(?=")' | head -1)
...
TMDBID=$(curl -s "https://www.themoviedb.org/search?query=$SEARCH" | cat | grep -oP -m 1 '(?<=class="result" href="/movie/).*?(?=">)' | head -1)
...
sixteen () {
  ...
  convert -size 5120x2160 xc:black \
    png/Widescreen/bbfc-logo.png -gravity northwest -geometry +741+201  -composite \
    ...
    png/Widescreen/"${RATING^^}".png -gravity northwest -geometry +639+698 -composite \
    ...
    png/Widescreen/President.png -gravity northwest -geometry +708+1768  -composite \
    png/Widescreen/Chief_Executive.png -gravity northwest -geometry +1431+1766  -composite \
    ...
    -fill white -font ProximaNova-Regular -pointsize 50 -kerning 5 -draw ' text 4102,1919 "'"${BBFCref^^}"'"' \
    Export/"$TMDBID".png
 ...
 fifteen () {
    ...
    -fill white -font Comic-Sans-MS -pointsize 50 -kerning 21.5 -draw ' text 2825,205 "'"${BBFCref^^}"'"' \
    ...
    -fill white -font RockwellStd-Bold -pointsize 85 -kerning 0 -interline-spacing -4 -draw 'text '$POSITION' "'"${TITLE^^}"'"' \
    ...
    png/Widescreen/President.png -gravity northwest -geometry +708+1768  -composite \
    Resources/Signatures/Cooke.png -gravity northwest -geometry +1322+1796  -composite \
